...
class DataLoaderX(DataLoader):
    def __iter__(self):
        return BackgroundGenerator(super().__iter__())  #병목현상 없애려고 함
...
def _dataset_factory(is_train, is_val):
    if is_val:
        ...
        print(cfg.train_dataset_module.shape)
        print(cfg.train_dataset_path)
        print(np.array(cfg.train_dataset_module).shape)
    else:
...
def make_data_sampler(dataset, shuffle, is_distributed):    #분산학습 여부
    if is_distributed:
